export class RefillingTokenBucket<_Key> {
  ...
  constructor(max: number, refillIntervalSeconds: number) {
    ...
    if (refillIntervalSeconds <= 0)
      throw new Error('refillIntervalSeconds must be positive');
    this.max = max;
  ...
  public consume(key: _Key, cost: number): boolean {
    // TODO: add error handling for edge cases like the one below
    if (cost > this.max) {
      throw new Error('Cost cannot be greater than maximum bucket size');
    }
...
export class Throttler<_Key> {
  ...
  constructor(timeoutSeconds: number[]) {
    if (timeoutSeconds.length === 0)
      throw new Error('timeoutSeconds must be non-empty');
